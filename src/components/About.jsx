import React from 'react'

function About() {
  return (
    <div id='about' className='w-full md:h-screen text-black p-8 '>
      <div className="md:w-full md:h-screen md:flex flex-col justify-center">
        <h1 className='text-3xl font-bold underline p-4 md:py-8'>About Us</h1>
        <p className="font-semibold text-lg">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Officia quo quis amet fugit voluptatem blanditiis, laboriosam reprehenderit nam fugiat itaque nemo similique nobis, odio a magnam. Libero, consectetur facilis. Animi!
          Ipsa deleniti eum id numquam quaerat ducimus neque maxime dignissimos fugiat quasi. Ratione eos nulla obcaecati aperiam, provident officia architecto in error eaque porro, ut eveniet id? Cum, omnis voluptates.
        </p>
        <br />
        <br />
        <p className="font-semibold text-lg">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Officia quo quis amet fugit voluptatem blanditiis, laboriosam reprehenderit nam fugiat itaque nemo similique nobis, odio a magnam. Libero, consectetur facilis. Animi!
          Ipsa deleniti eum id numquam quaerat ducimus neque maxime dignissimos fugiat quasi. Ratione eos nulla obcaecati aperiam, provident officia architecto in error eaque porro, ut eveniet id? Cum, omnis voluptates.
          Aliquam, tenetur sunt odit rem fugiat eveniet vel ut minima optio aliquid, soluta maiores fugit nihil veritatis quasi dolorem quibusdam! Unde iure doloribus corporis eligendi, quia aut provident voluptatum debitis?
        </p>
      </div>
    </div>
  )
}

export default About